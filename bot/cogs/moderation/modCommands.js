...
export const warn = {
  ...
  async execute(client, interaction) {
    ...
    if (target.user?.send) await target.user.send({ embeds: [buildEmbed({ type: 'warning', title: `Warning in ${interaction.guild.name}`, description: `**Reason:** ${reason}\n**Total warnings:** ${warnCount}` })] }).catch(() => {});
...
export const clear = {
  ...
  async execute(client, interaction) {
    ...
    let messages = interaction.channel?.messages ? await interaction.channel.messages.fetch({ limit: 100 }).catch(() => new Map()) : new Map();
    if (user)  messages = messages.filter(m => m?.author?.id === user.id);
    if (bots)  messages = messages.filter(m => m?.author?.bot);
    const cutoff = Date.now() - 14 * 86400000;
    const valid  = [...messages.values()].filter(m => m && m.createdTimestamp > cutoff).slice(0, amount);
    ...
    const deletedCount = deleted?.size ?? valid.length;
    return interaction.editReply({ embeds: [buildEmbed({ type: 'success', description: client.i18n.t('moderation.clear.success', { count: deletedCount }, lang) })] });
  },
};
