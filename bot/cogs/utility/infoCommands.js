...
function readablePerms(perms) {
  ...
  const active = Object.entries(PERM_NAMES).filter(([bit]) => {
    if (typeof perms.has === 'function') return perms.has(BigInt(bit));
    const rawVal = perms?.bitfield !== undefined ? perms.bitfield : perms;
    const target = (typeof rawVal === 'number' || typeof rawVal === 'bigint' || typeof rawVal === 'string') ? BigInt(rawVal) : 0n;
    return (BigInt(bit) & target) === BigInt(bit);
  });
...
export const roleinfo = {
  ...
  async execute(client, interaction) {
    ...
    const permStr    = readablePerms(role.permissions);
