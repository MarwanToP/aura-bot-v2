...
export const role = {
  ...
  async execute(client, interaction) {
    ...
    const botMe  = interaction.guild.members.me || interaction.guild.members.cache.get(client.user?.id) || { roles: { highest: { position: 0 } } };
    ...
    if (sub === 'add' || sub === 'remove') {
      ...
      if (role.position >= (botMe?.roles?.highest?.position ?? 0)) {
        return interaction.editReply({
      ...
      if (role.position >= (interaction.member?.roles?.highest?.position ?? 0)) {
        return interaction.editReply({
    ...
    if (sub === 'all' || sub === 'bots') {
      ...
      // Fetch all members
      const all = await interaction.guild.members.fetch().catch(() => new Map());
      const filterFn = m => sub === 'bots' ? (m?.user?.bot && !m?.roles?.cache?.has(role.id)) : (!m?.user?.bot && !m?.roles?.cache?.has(role.id));
      const targets = typeof all?.filter === 'function' ? all.filter(filterFn) : new Map();
...
export const slowmode = {
  ...
  async execute(client, interaction) {
    ...
    const botPerms = typeof interaction.channel.permissionsFor === 'function' ? interaction.channel.permissionsFor(interaction.guild.members.me) : null;
