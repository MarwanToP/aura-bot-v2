...
export const voice = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s
      .setName('ai')
      .setDescription('Voice AI session management')
      .addStringOption(o =>
        o.setName('action')
          .setDescription('Start or stop Voice AI session')
          .setRequired(true)
          .addChoices(
      ...
      .addChannelOption(o => o.setName('log_channel').setDescription('Channel for voice session logs').addChannelTypes(ChannelType.GuildText))
    ),
  ...
  async execute(client, interaction) {
    ...
    if (sub === 'setup') {
      ...
      if (typeof GuildSettings.upsert === 'function') {
      ...
      } else if (typeof GuildSettings.findOrCreate === 'function') {
        const [s] = await GuildSettings.findOrCreate({
          where: { guildId: interaction.guildId },
          defaults: { tempVoiceEnabled: true, tempVoiceCreatorId: creatorChannel.id, tempVoiceCategoryId: category?.id },
        });
        if (s?.update) {
          await s.update({ tempVoiceEnabled: true, tempVoiceCreatorId: creatorChannel.id, tempVoiceCategoryId: category?.id }).catch(() => {});
        }
      }
