...
export const presenceUpdate = {
  ...
  async execute(client, oldPresence, newPresence) {
    await importAndRun('../systems/voice/voiceSystem.js', 'handlePresenceUpdate', client, oldPresence, newPresence);
  },
};
...
async function importAndRun(path, fn, ...args) {
  const absolutePath = path.replace('../systems/', '../../shared/systems/');
  try {
    const mod = await import(absolutePath);
    if (mod[fn]) await mod[fn](...args);
  } catch (err) {
    logger.debug(`[Event] ${absolutePath}::${fn}: ${err.message}`);
  }
}
...
async function handleStarboard(client, reaction, user) {
  try {
    ...
    const { buildEmbed } = await import('../../shared/utils/embedBuilder.js');
